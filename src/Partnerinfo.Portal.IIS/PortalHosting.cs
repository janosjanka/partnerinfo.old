...
namespace Partnerinfo.Portal.IIS
{
    public class PortalHosting : IDisposable
    {
        ...
        public Site FindSiteByDomain(string domain)
        {
            ...
            {
                if (string.Equals(manager.Sites[i].Name, domain, StringComparison.OrdinalIgnoreCase))
                {
                    return manager.Sites[i];
                }
            }
        ...
        public long CreateSite(string domain)
        {
            ...
            var site = manager.Sites.Add(siteName, Settings.Default.PortalPath, 80);
        ...
        private ApplicationPool FindAppPool(string domain)
        {
            ...
            for (int i = manager.ApplicationPools.Count - 1; i >= 0; i--)
            {
                if (string.Equals(manager.ApplicationPools[i].Name, appPoolName, StringComparison.OrdinalIgnoreCase))
                {
                    return manager.ApplicationPools[i];
                }
            }
        ...
        {
            string appPoolName = CreateFullName(domain);
            ApplicationPool appPool = manager.ApplicationPools.Add(appPoolName);
