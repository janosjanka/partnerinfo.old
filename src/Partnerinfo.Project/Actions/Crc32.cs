    ...
    internal static class Crc32
    {
        ...
        public static uint Compute(string text)
        {
            ...
            for (int i = text.Length; --i >= 0;)
            {
                ...
                unchecked
                {
                    loByte = (byte)(charCode & 0x00ff);
                    hiByte = (byte)(charCode >> 8);
                }
                oldCrc = UPDC(hiByte, oldCrc);
                oldCrc = UPDC(loByte, oldCrc);
            }
            return ~oldCrc;
        }
        ...
        /// </summary>
        private static uint UPDC(byte octet, uint crc)
        {
            return (s_crcTable[(crc ^ octet) & 0xff] ^ (crc >> 8));
        }
    }
}
